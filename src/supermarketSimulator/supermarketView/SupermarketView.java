...
/**
 * This class observes state and prints out information each time an event
 * happens.
 *
...
public class SupermarketView extends View {
	...
	public void printEvent() {
		...
		if (state.getQueueList().getFirst().getEventName() == "StartEvent") {
			...
			System.out.printf("%-10s %-20s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s\n",
					info[0], info[1], "---", info[3], info[4], info[5], info[6], info[7], info[8], info[9], info[10],
					info[11], info[12]);
		} else if (state.getQueueList().getFirst().getEventName() == "StopEvent") {
			System.out.printf("%-10s %-10s\n", info[0], info[1]);
			printResult();
		} else {
			System.out.printf("%-10s %-20s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s %-10s\n",
	...
	/**
	 * Prints the parameters from state.
	 */
	...
	private void printResult() {
		...
		System.out.println("\nRESULTAT\n==========\n");
		System.out.println("1) Av " + (int) res[0] + " kunder handlade " + (int) res[1] + " medan " + (int) res[2]
				+ " missades.\n");
		System.out.println("2) Total tid " + (int) res[3] + " kassor varit lediga: " + df.format(res[4])
				+ " te.\n   Genomsnittlig ledig kassatid: " + df.format(res[4] / res[3]) + " te. (dvs. "
				+ df.format(((res[4] / res[3]) / res[5]) * 100)
				+ "% av tiden från öppning tills sista kunden betalat).\n");
		System.out.println("3) Total tid " + (int) res[6] + " kunder tvingats köa: " + df.format(res[7])
